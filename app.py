...
for activity_type in selected_activities:
    if activity_type == "🏖️ 海邊活動":
        if score >= 70 and temp >= 25 and wind_speed < 8:
            act_list = ", ".join(ACTIVITY_TYPES[activity_type][:3])
            recommendations.append(f"{activity_type}：{act_list}")
    ...
    if weather_data:
        ...
        if not display_forecasts:
            ...
            for i, forecast in enumerate(display_forecasts, 1):
                ...
                expander_title = f"{color} 推薦 #{i}：{forecast['date'].strftime('%m月%d日')} {forecast['weekday']} - {forecast['level']} (評分 {forecast['score']:.0f})"
        ...
        for f in sorted_forecasts:
            activities_str = ' | '.join(f['activities']) if f['activities'] else '無特別推薦'
            export_data.append({
